--Joins and selects with lots of records, where projected attributes are the same as the selection attributes
...
--test selects on large data sets
SELECT DA.filler FROM DA WHERE DA.filler = '00099 string record';
CREATE INDEX DB(dkey);
SELECT DB.serial FROM DB WHERE DB.dkey > 50.0;
SELECT DB.serial FROM DB WHERE DB.dkey = 55.0;
DROP INDEX DB(dkey);
--select all without using star
SELECT DB.dkey, DB.filler, DB.ikey, DB.serial FROM DB WHERE DB.serial > 90;


-- test join where projection attributes are the same as the join attribute 
--SMJ
SELECT DB.serial, DA.serial FROM DA, DB WHERE DA.serial = DB.serial;

--INL: test for all combinations of indexes
CREATE INDEX DB(dkey);
SELECT DB.dkey, DA.dkey FROM DA, DB WHERE DA.dkey = DB.dkey;
DROP INDEX DB(dkey);
CREATE INDEX DA(dkey);
SELECT DB.dkey, DA.dkey FROM DA, DB WHERE DA.dkey = DB.dkey;
CREATE INDEX DB(dkey);
SELECT DA.dkey, DB.dkey FROM DA, DB WHERE DA.dkey = DB.dkey;

--SNL
SELECT DA.serial, DB.ikey FROM DA, DB WHERE DA.serial >= DB.ikey;

SELECT * FROM DA WHERE DA.filler = 'Hussein';
